...
bot.on('message', async (ctx) => {
  ...
  if (ctx.update.message.new_chat_members) {
    ctx.update.message.new_chat_members.forEach((newUser) => {
      ctx.reply(`Hello @${newUser.username}, please complete the captcha at https://jsecoin.com/telegram-captcha.php?u=${newUser.id} within the next 3 minutes and before posting a message. This is to prevent spam on the Telegram channel.`);
...
const checkCaptcha = ((userID) => {
  ...
  request(`https://jsecoin.com/telegram-captcha.php?c=${userID}`, function (error, response, body) {
      ...
      if (body === '1') {
		...
		if (error) {
      console.log('Error 52 checkCaptcha server error: '+error);
